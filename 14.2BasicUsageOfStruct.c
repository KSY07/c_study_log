...
struct person /* person is tag of structure */
{
...
int main() {
    ...
    flag = scanf("%f", &genie.height); // &(genie.height) // p.s >> scanf returns number of success input
    printf("%f\n", genie.height);

    /* Initialization */

    struct person princess = {"Naomi Scott", 18, 160.0f}; // struct initialization must be strict with sequence
    ...
    someone->age = 1001; // arrow operator (pointer indirect)
